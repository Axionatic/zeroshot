...
function computeParamOverrides(mergedOptions) {
  return mergedOptions.skipQualityGate ? { quality_gate: false } : undefined;
}

function resolveParameterizedConfigFile(config, paramOverrides) {
...
function loadClusterConfig(
  ...
  const config = resolveParameterizedConfigFile(
    orchestrator.loadConfig(configPath),
    paramOverrides
  );
...
// Foreground `zeroshot run` pre-resolves the config (to apply strict-schema /
// model overrides) before handing it to startClusterFrom*. That bypasses the
// paramOverrides threading in resolveConfigOrThrow, so derive and apply the
// overrides here from the run options (incl. ZEROSHOT_RUN_OPTIONS in daemon mode).
function loadForegroundConfig({
  orchestrator,
  configPath,
  settings = {},
  providerOverride,
  ...
  return loadClusterConfig(
    ...
    settings,
    providerOverride,
    computeParamOverrides(mergeRunOptions(options))
  );
}
...
function buildStartOptions({
  ...
  return {
    ...
    paramOverrides: computeParamOverrides(mergedOptions),
...
function resolveConfigOrThrow({
  ...
  return loadClusterConfig(orchestrator, resolvedPath, settings, providerOverride, paramOverrides);
}
...
function startClusterWithInput(args, input) {
  ...
  const resolvedConfig = resolveConfigOrThrow({
    ...
    settings,
    providerOverride,
    paramOverrides: computeParamOverrides(mergeRunOptions(args.options || {})),
  });
